...
def set_background():
    if 'HTTP_X_FORWARDED_FOR' in request.httprequest.environ:
        ip = request.httprequest.environ["HTTP_X_FORWARDED_FOR"] if request else 'n/a'
    else:
    ...
    if IPv4Network(ip).is_private:
        request.params['cardmode'] = True
...
class LoginHome(Home):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if IPv4Network(ip).is_private:
            kw.update(dict(cardmode=True))
        # _logger.info("Login from %s:%s:%s" % (ip, kw, request.params.copy()))
